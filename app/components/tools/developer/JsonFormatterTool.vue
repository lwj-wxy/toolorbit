    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="flex flex-col space-y-3">
        <div class="flex items-center justify-between">
          ...
          <div class="space-x-2">
            <button
              class="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-md font-medium hover:bg-indigo-100 transition-colors"
              @click="formatJson(2)"
            >
              格式化 (2空格)
            </button>
      ...
      <div class="flex flex-col space-y-3">
        ...
        <div class="flex-1 relative">
          ...
          <div
            v-if="error"
            class="absolute inset-x-0 bottom-0 p-4 bg-red-50 text-red-700 text-sm font-medium border-t border-red-200 rounded-b-lg"
          >
            Error: {{ error }}
          </div>
        </div>
