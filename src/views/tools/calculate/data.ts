...
export const CALCULATE_TOOL_OVERVIEWS: Record<string, BilingualOverview> = {
  ...
  'rmb-converter': {
    zh: {
      ...
      input:
        '需要转换的人民币数字金额字符串，支持整数和最多两位小数，例如 1234.56、1000000.00、0.08、987654321.99 等；也支持纯整数输入（如 12000），工具将自动以"整"结尾。输入框会实时清理非数字字符，只保留一个小数点，并把小数部分限制在两位以内；整数部分按页面限制截取到约 13 位数字。页面提供常用金额快捷填充按钮（如 100、1000、10000、100000、1000000），方便快速测试或批量核对文档中的典型金额表达。',
      output:
      ...
      processing:
        '在浏览器内将金额数值拆分为整数部分和小数部分分别处理。整数部分按从高位到低位扫描，结合"仟佰拾"数位体系和"万 / 亿 / 兆"分组单位逐位映射为对应大写汉字（零/壹/贰/叁/肆/伍/陆/柒/捌/玖），连续零通过 zeroCount 逻辑合并为必要的单个"零"。小数部分按角（十分位）、分（百分位）逐一转换，角位为零且分位非零时补"零"，角位和分位均为零时追加"整"字。输入展示层先清理字符和小数位，转换函数再用 parseFloat 与 toFixed(2) 统一金额精度。所有校验和转换均在浏览器本地同步完成，输入金额不经过任何网络传输。',
      modes: ['阿拉伯数字 → 大写金额', '元角分标准格式', '整字规则处理', '零字合并与规范化', '亿/万/兆进位单位', '输入字符清理', '快捷金额测试', '一键复制结果'],
      example: {
        title: '人民币大写转换示例',
        input: '输入金额: 1234.56',
        output: '壹仟贰佰叁拾肆元伍角陆分',
    ...
    en: {
      ...
      input:
        'A RMB numeric amount string with an integer part and up to two decimal places, such as 1234.56, 1000000.00, 0.08, or 987654321.99. Pure integer input is also supported (e.g. 12000), and the tool automatically adds the 整 suffix. The input field cleans non-numeric characters in real time, keeps only one decimal point, limits the decimal part to two digits, and caps the integer part at roughly 13 digits. Quick-fill buttons for common amounts (100, 1000, 10000, 100000, 1000000) are provided for quick testing or document checks.',
      output:
      ...
      processing:
        'Splits the amount into integer and decimal parts and processes each separately in the browser. The integer part is scanned from high to low, mapping each digit to the corresponding uppercase Chinese character (零/壹/贰/叁/肆/伍/陆/柒/捌/玖), combining it with the 仟佰拾 digit units and the 万 / 亿 / 兆 group units. A zero-counting pass inserts only the needed 零 characters between non-zero groups. The decimal part is converted digit by digit as jiao (tenths) and fen (hundredths). When jiao is zero and fen is non-zero, a 零 is inserted; when both jiao and fen are zero, the 整 suffix is appended. The input field cleans characters and decimal precision before conversion, while the converter normalizes the numeric value with parseFloat and toFixed(2). All conversion runs synchronously in the local browser; the entered amount is never transmitted over the network.',
      modes: ['Arabic numerals to uppercase', 'Yuan/Jiao/Fen standard format', 'Integer ending rule (整)', 'Zero merging and normalization', 'Yi/Wan/Zhao carry units', 'Input character cleanup', 'Quick amount presets', 'One-click copy result'],
      example: {
  ...
  'ppi-calculator': {
    zh: {
      summary:
        'PPI（Pixels Per Inch，每英寸像素数）计算器用于根据显示设备的横向物理像素数、纵向物理像素数和对角线屏幕尺寸快速计算像素密度，帮助评估和对比不同屏幕的显示精细程度。适合前端开发者和 UI 设计师在设计稿适配时参照目标设备的像素密度（确保@2x/@3x 切图与实际 PPI 匹配）、硬件测评人员对比手机、笔记本和显示器屏幕的清晰度指标、IT 采购者在选购显示器时快速比较不同尺寸与分辨率组合，以及需要理解 Retina / HiDPI / 高分屏等概念的初学者快速上手计算。工具内置 iPhone 12/13/14、MacBook Pro 13、27 英寸 4K 显示器、24 英寸 1080P 办公屏等快捷预设，并提供基于 PPI 区间的清晰度等级评定。',
      input:
        '屏幕宽度像素（如 1920、2560、3840 等）、屏幕高度像素（如 1080、1440、2160 等）和对角线物理尺寸（以英寸为单位，如 6.1、13.3、24、27 等）。用户可手动输入任意设备的三个关键参数，也可通过内置的常见设备预设（iPhone 12/13/14、MacBook Pro 13、27" 4K 桌面显示器、24" 1080P 办公屏）一键填充。支持小数英寸输入，适应不同设备对角线尺寸的实际规格。',
      output:
        '计算得到的 PPI 数值（保留两位小数），以及根据像素密度区间给出的直观清晰度等级提示。当前页面按固定阈值分类：PPI ≥ 300 判定为高清晰度 / Retina 级，PPI 200-299 判定为良好清晰度，PPI 100-199 判定为普通清晰度，PPI < 100 判定为较低清晰度。输出结果适合作为设备选型对比、设计适配参考和显示品质初步判断的依据。',
      processing:
        '使用勾股定理在浏览器内同步计算。首先根据用户输入的横向像素数（Width）和纵向像素数（Height）计算屏幕总像素对角线长度：sqrt(Width² + Height²)；然后将像素对角线值除以物理对角线英寸数（Diagonal）得到 PPI = sqrt(Width² + Height²) / Diagonal。例如一台 15.6 英寸、1920×1080 分辨率的笔记本，PPI = sqrt(1920² + 1080²) / 15.6 ≈ 141.21。页面根据计算结果所在区间自动映射对应的清晰度等级标签。所有计算仅依赖用户输入的三个数值，在浏览器前端同步完成，无需网络连接。',
      modes: ['宽/高像素输入', '对角线英寸尺寸输入', '常见设备预设一键填充', 'PPI 计算值', '清晰度等级评定（Retina/高/中/低）'],
      example: {
        title: 'PPI 计算示例',
        input: '屏幕宽度: 1920 px\n屏幕高度: 1080 px\n对角线: 15.6 in',
    ...
    en: {
      summary:
        'The PPI (Pixels Per Inch) Calculator computes pixel density from a display device\'s horizontal physical pixel count, vertical physical pixel count, and diagonal screen size, helping evaluate and compare display sharpness. It is useful for frontend developers and UI designers checking target-device density, hardware reviewers comparing phones, laptops, and monitors, IT purchasers comparing monitor size/resolution combinations, and beginners learning Retina / HiDPI concepts. The tool includes quick presets for iPhone 12/13/14, MacBook Pro 13, a 27-inch 4K monitor, and a 24-inch 1080P office monitor, plus a clarity rating based on fixed PPI ranges.',
      input:
        'Screen width in pixels (e.g. 1920, 2560, 3840), screen height in pixels (e.g. 1080, 1440, 2160), and diagonal physical size in inches (e.g. 6.1, 13.3, 24, 27). Users can manually enter the three key parameters for any device or use built-in presets for iPhone 12/13/14, MacBook Pro 13, a 27" 4K desktop monitor, or a 24" 1080P office monitor. Decimal inch input is supported.',
      output:
        'The calculated PPI value is shown with two decimal places plus an intuitive clarity rating based on fixed ranges. PPI ≥ 300 is rated High / Retina-grade, PPI 200-299 is rated Good clarity, PPI 100-199 is rated Standard clarity, and PPI < 100 is rated Lower clarity. Results serve as a reference for device comparison, design adaptation guidance, and preliminary display quality assessment.',
      processing:
        'Calculates synchronously in the browser using the Pythagorean theorem. First the total screen pixel diagonal is computed from the entered horizontal pixel count (Width) and vertical pixel count (Height): sqrt(Width² + Height²). Then the pixel diagonal is divided by the physical diagonal size in inches (Diagonal) to obtain PPI = sqrt(Width² + Height²) / Diagonal. For example, a 15.6-inch, 1920×1080 laptop gives PPI ≈ 141.21. The page maps the computed result to the corresponding clarity rating label based on its range. All calculation depends only on the three user-entered values and runs synchronously in the browser frontend with no network connection required.',
      modes: ['Width/height pixel input', 'Diagonal inch size input', 'Common device preset one-click fill', 'PPI calculated value', 'Clarity rating (Retina/Good/Standard/Low)'],
      example: {
