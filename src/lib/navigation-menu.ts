...
export function getNavigationMenuData(): NavigationMenuData {
  const navigationTools = TOOLS_META.filter((tool) => !tool.isNoIndex);
  ...
  const categories = Array.from(new Set(navigationTools.map((tool) => tool.category)))
    .filter((category) => category !== 'AI 工具')
    .map((category) => ({
      category,
      path: getCategoryPath(category),
      tools: navigationTools.filter((tool) => tool.category === category).map(toNavTool),
    }));

  return {
    categories,
    aiCategoryPath: getCategoryPath('AI 工具'),
    aiTools: navigationTools.filter((tool) => tool.category === 'AI 工具').map(toNavTool),
  };
}
