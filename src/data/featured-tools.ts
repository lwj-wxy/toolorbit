...
export const FEATURED_TOOLS: FeaturedTool[] = [
  // ─── Developer Tools ──────────────────────────────
  {
    ...
    descriptionZh: [
      "微软出品的免费开源代码编辑器，拥有庞大的插件生态、内置终端和一流的 Git 支持，是前端和后端开发者的常用编辑器。",
      "VS Code 内置智能感知、调试器、Git 命令和数千个社区扩展，覆盖所有主流语言和框架。Remote Development 套件支持在容器、WSL 和远程 SSH 环境中顺畅开发。",
  ...
  },
  {
    ...
    description: [
      ...
      "GitHub Actions enables automated CI/CD pipelines triggered by push, PR, or schedule, with thousands of reusable workflow templates. GitHub Copilot, Codespaces, and advanced code search round out a broad development platform.",
      "Solo developers and teams use it for version control, open-source collaboration, GitHub Pages hosting, issue tracking, and automated deployment. The free tier covers many personal and small-team projects.",
    ],
    descriptionZh: [
      "全球最大的代码托管平台，提供 Git 版本控制、Actions CI/CD 流水线、项目管理与协作代码审查，是开源社区的常用入口。",
      "GitHub Actions 支持通过 push、PR 或定时触发自动化的 CI/CD 流水线，提供数千个可复用的工作流模板。Copilot、Codespaces 和高级代码搜索让 GitHub 成为覆盖完整的开发平台。",
      "独立开发者和团队都常用。可用于版本控制、开源协作、GitHub Pages 作品集托管、Issue 追踪和自动化部署。免费额度对大多数项目足够可用。",
    ],
    slug: 'github.com',
  ...
  },
  {
    ...
    descriptionZh: [
      "API 开发与测试平台，支持 REST、GraphQL 和 gRPC 接口的设计、调试、文档生成与 mock 服务，是后端联调工具。",
      "Postman 提供环境变量、前置脚本、基于 Chai 断言的测试自动化和 CI 集成的集合运行器。Mock 服务器功能让前端团队在后端就绪前即可基于稳定的 API 契约并行开发。",
      "后端开发者、API 设计者和 QA 工程师的常用工具。可用于探索第三方 API、文档化内部服务、运行回归测试、以及从 OpenAPI 规范生成客户端 SDK。",
    ],
    slug: 'postman.com',
  ...
  },
  {
    ...
    description: [
      ...
      "Frontend teams use it to deploy React, Next.js, Vue, Svelte, and static sites. GitHub integration and preview URLs make branch review simple before release.",
    ],
    descriptionZh: [
      ...
      "前端开发者的理想部署平台，支持 React、Next.js、Vue、Svelte 和静态站点。可用的免费额度和顺畅 GitHub 集成使其成为交付副项目、营销页和 SaaS 前端的常用。",
    ],
    slug: 'vercel.com',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "Docker Compose 通过单个 YAML 文件编排多容器应用，Docker Hub 提供数百万预构建镜像的注册中心。层缓存机制加速迭代构建。",
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "前端开发者原型验证动画、CSS 布局和 UI 组件的工具。广泛用于分享可复现的 Bug 报告、教学 Web 概念和在博客中嵌入交互式示例。",
    ],
    slug: 'codepen.io',
  ...
  },
  {
    ...
    description: [
      ...
      "Stack Overflow uses reputation, tags, voting, and moderation to keep useful answers visible. Collectives connect vendors with developer communities, and Stack Overflow for Teams provides private Q&A.",
      "Developers use it to debug errors, compare trade-offs, find API examples, and learn how other engineers solved similar problems.",
    ],
    descriptionZh: [
      ...
      "每个开发者调试时的常用入口。除排查问题外，还可用于发现最佳实践、对比技术方案的利弊，以及学习资深工程师处理真实问题的方式。",
    ],
  ...
  },
  {
    ...
    description: [
      ...
      "Railway offers instant GitHub deploys, one-click database provisioning, environment variable management, and usage-based billing with a useful free starter credit. The CLI and API enable full infrastructure-as-code workflows.",
    ...
    descriptionZh: [
      ...
      "Railway 提供即时 GitHub 部署、一键创建数据库、环境变量管理和基于用量的计费，附带可用的免费启动额度。CLI 和 API 支持完整的基础设施即代码工作流。",
  ...
  // ─── Design Resources ──────────────────────────────
  {
    ...
    description: [
      ...
      "Figma supports real-time multiplayer editing, Dev Mode handoff, plugins for accessibility checks and asset export, component variants, and auto-layout.",
      "Designers, product managers, and frontend developers use it to create wireframes, design systems, prototypes, and specs in one shared file.",
    ],
    descriptionZh: [
      "基于浏览器的协作式 UI/UX 设计工具，支持矢量编辑、交互原型、组件库和 Dev Mode 开发交付，已成为行业标准设计平台。",
      "Figma 的云端优先架构支持实时多人编辑、Dev Mode 顺畅交付，以及丰富的插件生态（无障碍检查、内容生成、资源导出）。组件变体和自动布局加速设计迭代。",
  ...
  },
  {
    ...
    description: [
      ...
      "Designers, creative directors, and product teams browse it for UI patterns, illustration styles, branding concepts, and motion design references.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    description: [
      ...
      "A common tool for UI designers, frontend developers, and brand designers who need quick, cohesive color schemes. Use the contrast checker to ensure WCAG compliance, export palettes directly into design tools, and build a library of project color sets.",
    ],
    descriptionZh: [
      "快速配色方案生成器，按空格键即可生成协调的调色板，支持锁定颜色、调整色阶和导出，是设计配色效率工具。",
      "Coolors 通过按空格键即可生成和谐的配色方案。可锁定喜欢的颜色、调整色阶、检查无障碍对比度，并导出为 CSS、SCSS、SVG、PDF 或图片，全部即时完成。",
      "UI 设计师、前端开发者和品牌设计师获取快速协调配色的常用工具。使用对比度检查器确保 WCAG 合规、直接导出到设计工具、建立项目配色库。",
    ],
    slug: 'coolors.co',
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for developers building website mockups, designers creating social media graphics, content creators illustrating blog posts, and product teams needing placeholder imagery. The API enables programmatic image access for applications.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    description: [
      ...
      "Google serves the fonts from its global CDN and negotiates modern formats for each browser.",
      "A common web font resource for frontend developers and designers. Use it to select readable body text, distinctive headings, and monospace code fonts. Variable fonts reduce the number of HTTP requests while enabling fine-grained typographic control.",
    ],
    descriptionZh: [
      "免费开源的 Web 字体库，提供 1500+ 字体家族，支持可变字体和 CSS 嵌入代码，是 Web 排版的常用资源。",
      "Google Fonts 提供 1500+ 开源字体家族，支持可变字体、性能优化的子集以及简洁的 CSS/HTML 嵌入代码。字体从 Google 全球 CDN 分发，自动为现代浏览器协商最佳格式。",
      "前端开发者和设计师的 Web 字体常用资源。可用于选择可读性强的正文、有特色的标题和等宽代码字体。可变字体在减少 HTTP 请求的同时实现精细的排版控制。",
    ],
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "适合工程师绘制架构图、产品经理梳理用户流程以及教育者制作可视化讲解。手绘美学降低了追求适合的压力，鼓励迭代式思考。",
    ],
    slug: 'excalidraw.com',
  ...
  // ─── Productivity ──────────────────────────────────
  {
    ...
    titleZh: 'Notion',
    description: [
      "All-in-one workspace for notes, docs, databases, wikis, and project management with capable block-based editing and team collaboration.",
      ...
      "Useful for individuals managing personal knowledge bases, startups documenting internal processes, and teams running lightweight project management. The template gallery provides quick-start setups for meeting notes, product roadmaps, habit trackers, and more.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    description: [
      ...
      "Obsidian stores notes as local plain Markdown files, giving you full ownership of your data. Bidirectional links, the graph view, and a capable plugin ecosystem turn your notes into an interconnected knowledge network, a true \"second brain.\"",
      "Useful for researchers, writers, and developers who want a durable, portable knowledge base. Use it to build interconnected study notes, document technical concepts with code snippets, and create Zettelkasten-style idea networks.",
    ],
    descriptionZh: [
      "基于本地 Markdown 文件的知识库工具，支持双向链接、图谱视图和丰富的插件生态，适合打造个人第二大脑。",
      "Obsidian 将笔记存储为本地纯 Markdown 文件，让你完全掌控自己的数据。双向链接、图谱视图和强大的插件生态将你的笔记转化为互联的知识网络，\"第二大脑\"。",
  ...
  },
  {
    ...
    titleZh: 'Raycast',
    description: [
      "Fast macOS launcher and productivity toolkit with extensions for clipboard history, window management, snippets, and API integrations.",
      ...
      "Useful for macOS-using developers and power users. Use it to compose Jira tickets with a shortcut, search GitHub repos without opening a browser, manage window layouts, convert units, format JSON, all without leaving the keyboard.",
    ],
    descriptionZh: [
      "流畅流畅的 macOS 启动器与效率工具箱，集成剪贴板历史、窗口管理、代码片段和 API 扩展，减少日常操作耗时。",
  ...
  },
  {
    ...
    description: [
      ...
      "A common diagramming tool for engineers, architects, and technical writers. Use it to document system architectures, map database schemas, sketch API flows, and create presentation-ready diagrams for design reviews and RFCs.",
    ],
    descriptionZh: [
      "免费在线绘图工具，支持流程图、UML、网络拓扑和组织架构图，可完全离线使用并集成云存储，是技术绘图综合工具。",
      "draw.io 提供无需注册、无云锁定的专业绘图体验。支持流程图、UML 图、网络拓扑图、组织架构图和 BPMN，完全支持离线使用并集成 Google Drive、OneDrive 和 GitHub。",
      "工程师、架构师和技术写作者的常用绘图工具。可用于记录系统架构、绘制数据库 Schema、草拟 API 流程以及为设计评审和 RFC 创建可用于演示的图表。",
    ],
    slug: 'diagrams.net',
  ...
  // ─── SEO & Marketing ───────────────────────────────
  {
    ...
    description: [
      ...
      "Google Search Console shows which queries drive traffic, which pages are indexed, and whether Google sees mobile usability or Core Web Vitals issues. URL Inspection shows how Googlebot renders a page.",
    ...
    descriptionZh: [
      "Google 官方免费工具，监控网站索引状态、搜索表现、Core Web Vitals 和人工处理措施，是每个网站运营者的常用工具。",
      ...
      "每个网站所有者、SEO 专家和内容营销人员的常用工具。可用于提交 Sitemap、监控新内容索引情况、识别点击率低下的页面以及接收 Google 检测到的网站问题告警。",
    ],
  ...
  },
  {
    ...
    titleZh: 'Ahrefs',
    description: [
      "broad SEO platform with backlink analysis, keyword research, rank tracking, and content gap tools, widely used by SEO professionals.",
      "top pages, organic keywords, and paid search strategies.",
  ...
  },
  {
    ...
    descriptionZh: [
      "桌面端 SEO 爬虫工具，审计页面元素、重定向、死链、重复内容和 hreflang 标签，是技术 SEO 审计的行业工具。",
      ...
      "代理商和内部 SEO 团队的技术 SEO 审计常用工具。适用于网站迁移前、重设计 QA 期间和季度健康检查。自定义提取功能可抓取任意 HTML 元素进行批量分析。",
    ],
  ...
  },
  {
    ...
    description: [
      ...
      "A useful ideation tool for content marketers, SEO writers, and product teams. Use it to build FAQ sections that answer real questions, discover content angles competitors missed, identify product feature opportunities from user intent, and fuel blog editorial calendars.",
    ],
    descriptionZh: [
      ...
      "内容营销人员、SEO 写手和产品团队的灵感工具。可用于构建回答真实问题的 FAQ 板块、发现竞品遗漏的内容角度、从用户意图中识别产品功能机会以及充实博客选题日历。",
    ],
  ...
  // ─── AI Tools ──────────────────────────────────────
  {
    ...
    titleZh: 'Claude',
    description: [
      "Anthropic's AI assistant for long-context reasoning, code generation, and document analysis, available through claude.ai and API.",
  ...
  },
  {
    ...
    titleZh: 'ChatGPT',
    description: [
      "OpenAI's conversational AI platform with GPT models, image generation, data analysis tools, custom GPTs, and voice features.",
      "ChatGPT gives users access to OpenAI models, data analysis in a Python sandbox, image generation, web browsing, custom GPTs, and mobile voice conversations.",
    ...
    descriptionZh: [
      "OpenAI 的对话式 AI 平台，集成了 GPT-4 推理、DALL·E 图像生成、代码解释器和自定义 GPTs，是常见的 AI 助手。",
      "ChatGPT 提供对 OpenAI 最强模型的访问，功能包括代码解释器（Python 沙箱）、DALL·E 图像生成、网页浏览、自定义 GPT 以及支持语音对话的移动应用。插件和 GPT Store 生态扩展了功能边界。",
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "ML 工程师、研究者和 AI 从业者的常用入口。可用于发现成熟的模型、在自定义数据上微调预训练权重、与利益相关者分享模型演示以及接入生产级推理端点。",
    ],
    slug: 'huggingface.co',
  ...
  },
  {
    ...
    description: [
      ...
      "A common research tool for knowledge workers who need verifiable answers. Use it to research market trends with current data, verify technical claims with citations, summarize recent news on any topic, and conduct literature reviews with actual paper references.",
    ],
    descriptionZh: [
      ...
      "需要可验证答案的知识工作者的常用研究工具。可用于研究带当前数据的市场趋势、用引用验证技术声明、总结任意话题的最新动态以及进行有真实文献参考的综述。",
    ],
    slug: 'perplexity.ai',
  ...
  },
  {
    ...
    description: [
      ...
      "Midjourney generates images from text prompts with high aesthetic quality, known for its artistic, painterly style and precise control through parameters like aspect ratio, stylization, and chaos. The platform operates through Discord, fostering a vibrant creative community.",
      "A common creative tool for designers exploring visual concepts, marketers creating social media assets, game developers prototyping character and environment art, and artists experimenting with AI-assisted workflows. The style reference feature enables consistent brand-aligned visual output.",
    ],
    descriptionZh: [
      "通过 Discord 使用的 AI 图像生成平台，以高美学品质、艺术风格控制和风格参考功能著称，是设计师和创意者的工具。",
      "Midjourney 通过文本提示生成图像，以较高的美学品质著称，艺术性的绘画风格以及通过宽高比、风格化和混沌度等参数实现的精确控制。平台通过 Discord 运作，培育了活跃的创意社区。",
  ...
  },
  {
    ...
    description: [
      ...
      "A useful tool for frontend developers and designers prototyping UI rapidly. Describe a dashboard card, pricing table, or signup form in plain English and get working React code in seconds, then iterate with follow-up prompts to refine the design.",
    ],
    descriptionZh: [
      ...
      "前端开发者和设计师快速原型 UI 的实用工具。用简单英语描述仪表盘卡片、定价表或注册表单，即可在秒级获得可运行的 React 代码，然后通过后续提示迭代优化设计。",
    ],
    slug: 'v0.dev',
  ...
  // ─── Ecommerce Tools ──────────────────────────────
  {
    ...
    description: [
      ...
      "Shopify provides everything needed to launch and scale an online store, customizable themes, integrated payment processing, inventory management, shipping, analytics, and a large app marketplace for extending functionality. It powers millions of businesses from solo entrepreneurs to global brands.",
    ...
    descriptionZh: [
      ...
      "推出直销品牌的零售创业者的常用平台。可用于数小时内搭建商店、管理多渠道销售（线上、社交、线下 POS）、运营邮件营销活动以及通过 Shopify Capital 获得运营资金。",
    ],
    slug: 'shopify.com',
  ...
  },
  {
    ...
    description: [
      ...
      "The calculator shows each Etsy fee category separately. Adjust sale price, shipping cost, and quantity to see net profit, total fees, and effective fee percentage.",
    ...
    descriptionZh: [
      "面向开发者的支付处理平台，提供在线支付、订阅、发票和风控 API，支持 135+ 货币，是 SaaS 和电商集成的常用。",
      "计算器逐项拆解每种费用类别，清晰展示 Etsy 的费用结构如何影响你的实际利润。调整售价、运费和数量即可即时查看更新的净利润、总费用和实际费率。",
      "Etsy 卖家规划库存、开展促销和分析盈利能力的常用定价工具。可用于对比不同价位的利润率、将运费纳入定价策略以及避免定价过低侵蚀实际收入。",
    ],
    slug: 'stripe.com',
  ...
  // ─── Learning Resources ────────────────────────────
  {
    ...
    description: [
      ...
      "MDN Web Docs covers HTML, CSS, JavaScript, SVG, Web APIs, and HTTP. Its pages include compatibility data, interactive examples, and links to relevant standards.",
    ...
    descriptionZh: [
      ...
      "MDN Web Docs 是 Web 技术覆盖较广、更新稳定的参考文档，覆盖 HTML、CSS、JavaScript、SVG、Web API 和 HTTP。每页均包含浏览器兼容性数据、交互式代码示例和相关标准链接。",
  ...
  },
  {
    ...
    descriptionZh: [
      "资深 CSS 技术博客，提供深入的 CSS 布局、动画和响应式设计教程与速查手册，是前端开发者进阶的资料库。",
      "CSS-Tricks 是十多年来长期维护的 CSS 博客，发布深入指南、详尽的 CSS 属性与选择器速查手册以及关于现代布局、动画和响应式设计模式的实用文章。",
  ...
  // ─── Open Source ──────────────────────────────────
  {
    ...
    descriptionZh: [
      ...
      "Web 开发的主流 UI 库，构建交互式用户界面的前端开发者常用。可用于创建可复用的组件层次结构、可预测地管理 UI 状态以及利用庞大的工具、库和社区知识生态。",
    ],
    slug: 'react.dev',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "构建生产级 Web 应用的 React 框架常用。可用于通过 SSR 优化 SEO、生成静态营销页面、在前端旁创建 API 端点以及在 Vercel 或任何 Node.js 服务器上一键部署。",
    ],
    slug: 'nextjs.org',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "Tailwind CSS 是 Utility-first CSS 框架，直接通过 \"flex items-center gap-4\" 等类名在 HTML 中构建设计。仅生成实际使用的 CSS、内置响应式变体且与组件框架顺畅集成。",
      "对想要快速构建自定义设计而无需在 HTML 和 CSS 文件之间切换的前端开发者来说具有实用。可用于快速原型布局、通过可配置主题强制执行设计一致性以及产出极小的生产 CSS 包。",
    ],
  ...
  },
  {
    ...
    descriptionZh: [
      "开源的 Firebase 替代方案，提供 Postgres 数据库、认证、实时订阅、存储和边缘函数，免费额度可用，适合快速搭建应用后端。",
      "Supabase 提供开源的 Firebase 替代方案，Postgres 数据库、实时订阅、认证、行级安全、文件存储和边缘函数。托管平台提供可用的免费额度，无规模扩展时的意外用量限制。",
      "想要 SQL 能力加实时特性的独立开发者和初创团队的适合后端。可用于构建即时数据同步的应用、数分钟实现社交登录、存储和提供用户生成内容以及无需构建 API 即可从浏览器查询数据。",
    ],
    slug: 'supabase.com',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "厌倦手动同步 API 类型的全栈 TypeScript 开发者的实用工具。可用于消除内部应用对 REST/GraphQL Schema 的需求、在 API 变更时获得即时反馈以及以完全的类型安全信心更快地构建。",
    ],
    slug: 'trpc.io',
  ...
  // ══════════════════════════════════════════════════════
  {
    ...
    descriptionZh: [
      ...
      "适合构建 CLI 工具、Serverless 函数和高吞吐量 API 的开发者。原生 TypeScript 支持和 npm 兼容的包管理器使现有项目可顺畅迁移以获得性能提升。",
    ],
    slug: 'bun.sh',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "QA 工程师和需要可靠端到端测试的全栈开发者常用。可用于验证关键用户流程、捕获跨浏览器回归问题以及为设计评审生成视觉快照。",
    ],
    slug: 'playwright.dev',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "TypeScript 开发者构建数据驱动应用的常用 ORM。它消除了数据库 Schema 与应用类型之间的鸿沟，加速 CRUD 开发，并与 Next.js、NestJS、tRPC 技术栈平滑集成。",
    ],
    slug: 'prisma.io',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "管理多包仓库的平台团队的常用工具。降低 CI 成本、缩短开发者反馈循环，并与现有 npm、pnpm 和 Yarn workspaces 透明集成。",
    ],
    slug: 'turbo.build',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "构建设计系统和组件库的团队常用。可用于向利益相关者展示组件、通过 Chromatic 运行视觉回归测试，以及让新开发者在不启动完整应用的情况下浏览每个 UI 状态。",
    ],
  ...
  },
  {
    ...
    descriptionZh: [
      "安全的内网穿透服务，一键将本地服务暴露到公网并自带 HTTPS，是 Webhook 调试和 Demo 演示的常用工具。",
      "ngrok 通过公共 URL 创建到本地主机的安全隧道，自动配置 HTTPS、请求检查与重放。预留域名和 TCP 隧道为预发布环境和 IoT 设备提供更持久的方案。",
      "测试 Webhook、OAuth 回调和支付通知的开发者工具。可用于向客户演示进行中的工作、结对编程时共享本地开发服务器，以及从防火墙后暴露 API。",
    ],
    slug: 'ngrok.com',
  ...
  },
  {
    ...
    description: [
      ...
      "Cloudflare Workers deploy JavaScript at the edge, within milliseconds of users worldwide, with zero cold starts and a useful free tier. The broader platform includes DDoS protection, DNS management, R2 object storage, D1 SQLite database, and AI inference APIs.",
    ...
    descriptionZh: [
      ...
      "Cloudflare Workers 在全球边缘部署 JavaScript，距用户仅毫秒延迟，零冷启动且免费额度可用。更广泛的平台包含 DDoS 防护、DNS 管理、R2 对象存储、D1 SQLite 数据库和 AI 推理 API。",
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "频繁在不同数据格式间切换的开发者的省时工具。可用于从 API 响应生成 TypeScript 类型、将 CSS 迁移为 Tailwind 工具类、将 JSON Schema 转为 TypeScript 接口以及探索不熟悉的格式。",
    ],
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "注重性能的前端开发者的常用工具。添加任何新依赖前先对比不同方案对打包体积的影响，识别臃肿的包，保持 JavaScript 负载精简。",
    ],
  ...
  },
  {
    ...
    description: [
      ...
      "Frontend developers check Can I Use before adopting modern web features. Use it to verify CSS properties, JavaScript APIs, browser versions, and support notes.",
    ],
    descriptionZh: [
      "最新的 HTML/CSS/JS/SVG/Web API 浏览器兼容性速查表，前端开发者判断特性可用性的常用参考站。",
      "Can I Use 提供每个 HTML、CSS、JavaScript、SVG 和 Web API 特性的详细浏览器支持表，包括全球使用率统计、已知问题和相关规范链接。数据随浏览器新版本发布而更新。",
      "每个前端开发者在使用现代 Web 特性前的常用参考。采用新 CSS 属性前先检查兼容性数据、验证 JavaScript API 在目标浏览器中的支持情况，在文档中包含支持说明。",
    ],
    slug: 'caniuse.com',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "集成第三方 API 的开发者的日常效率提升工具。可将 Chrome DevTools 的\"复制为 cURL\"输出转换为你的常用语言、翻译 API 文档示例以及调试 HTTP 请求。",
    ],
  ...
  },
  {
    ...
    descriptionZh: [
      "轻量级 TypeScript ORM，采用类 SQL 语法、零依赖和流畅类型安全，专为 Serverless 和边缘环境设计。",
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "设计师、电商卖家和营销人员的时间节省工具。可用于创建干净背景的产品照片、生成用于合成的透明 PNG 以及为团队页面和社交媒体准备头像。",
    ],
    slug: 'remove.bg',
  ...
  },
  {
    ...
    titleZh: 'TinyPNG',
    description: [
      "Smart lossy compression for WebP, PNG, and JPEG images, reduces file size while preserving visual quality.",
    ...
    descriptionZh: [
      "智能有损压缩 WebP/PNG/JPEG 图片，在保持视觉质量的前提下减小文件体积，是网页性能优化的常用工具。",
  ...
  },
  {
    ...
    titleZh: 'Squoosh',
    description: [
      "GoogleChromeLabs' open-source image compression web app with side-by-side comparison, format conversion, and codec options for MozJPEG, AVIF, and WebP.",
      ...
      "Useful for performance-focused developers choosing the optimal format and compression level for each image. Use it to benchmark AVIF vs WebP quality at different sizes, generate compressed assets for a CMS, and understand the trade-offs between codec options.",
    ],
    descriptionZh: [
      ...
      "注重性能的开发者选择最优图片格式和压缩级别的常用工具。可用于对比不同尺寸下 AVIF 与 WebP 的画质、为 CMS 生成压缩资源以及理解各编码器的权衡取舍。",
    ],
    slug: 'squoosh.app',
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for Tailwind CSS projects where visual consistency matters. Use outline icons for nav bars and form controls, solid icons for primary actions, and mini icons for dense UIs like data tables and breadcrumbs.",
    ],
    descriptionZh: [
      "Tailwind CSS 团队出品的精美手绘 SVG 图标，提供 outline、solid 和 mini 三种风格，原生支持 React 和 Vue。",
      "Heroicons 提供 300+ 免费 SVG 图标，三种风格，outline、solid 和 mini，全部基于统一的 24×24 网格设计。作为 Tailwind CSS 官方图标集，与 utility-first 工作流适合集成。",
  ...
  },
  {
    ...
    description: [
      ...
      "A common resource for designers and developers who need hero backgrounds, section dividers, and decorative elements without creating them from scratch. The SVG output is resolution-independent and lightweight, keeping page load fast.",
    ],
    descriptionZh: [
      ...
      "设计师和开发者获取首屏背景、章节分割线和装饰元素的常用资源，无需从零创作。SVG 输出分辨率无关且轻量，保持页面加载快速。",
    ],
    slug: 'haikei.app',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "远程团队报告 Bug、演示设计、录制 Sprint Demo 和创建入职指南的工具。用可观看、可回放的视频消息替代冗长的 Slack 讨论和会议。",
    ],
    slug: 'loom.com',
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for macOS users who create documentation, tutorials, or bug reports. Use it to capture full-page web content, annotate screenshots for design feedback, record step-by-step product demos, and quickly share visuals with a link.",
    ],
    descriptionZh: [
      ...
      "创建文档、教程或 Bug 报告的 macOS 用户常用。可用于截取整页网页内容、为设计反馈标注截图、录制分步产品演示以及快速用链接分享视觉内容。",
    ],
    slug: 'cleanshot.com',
  ...
  },
  {
    ...
    description: [
      ...
      "Slack organizes team communication into channels, with threaded replies, app integrations, and capable search. Workflow Builder automates routine processes like onboarding checklists and standup reminders without writing code.",
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for individuals and small teams practicing GTD (Getting Things Done). Use projects for areas of responsibility, labels for context (@home, @office), priorities for daily triage, and Karma for motivation and habit building.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for developers who version-control documentation alongside code. Embed Mermaid diagrams in README files, architecture decision records (ADRs), and API docs so diagrams stay up-to-date with code changes in every commit.",
    ],
    descriptionZh: [
      ...
      "将文档与代码一同纳入版本控制的开发者的常用工具。在 README、架构决策记录（ADR）和 API 文档中嵌入 Mermaid 图，使图表随代码变更在每次提交中保持最新。",
    ],
    slug: 'mermaid.js.org',
  ...
  // ══════════════════════════════════════════════════════
  {
    ...
    description: [
      ...
      "traffic sources, ad budgets, and content strategies, all in one dashboard.",
      "Digital marketing teams use it to research keyword difficulty, track daily SERP positions, audit on-page SEO, analyze competitor ad copy, and measure brand visibility across channels.",
    ],
    descriptionZh: [
      "一站式数字营销平台，覆盖 SEO、PPC、内容营销、社交媒体和竞品研究，提供企业级数据洞察。",
      "Semrush 提供 SEO、付费搜索、内容营销、社交媒体和竞品研究的统一平台。域名分析一站式展示竞品的流量来源、广告预算和内容策略。",
      "数字营销团队的综合工具。可用于研究关键词难度、追踪每日 SERP 排名、审计页面 SEO、分析竞品广告文案以及衡量跨渠道品牌曝光。",
    ],
    slug: 'semrush.com',
  ...
  },
  {
    ...
    description: [
      ...
      "SEOs and web developers use it to implement structured data. Look up property names for Article, Product, FAQ, Event, and Organization markup, then validate JSON-LD before deployment.",
    ],
    descriptionZh: [
      ...
      "实现结构化数据的 SEO 和 Web 开发者的常用参考。可用于查询 Article、Product、FAQ、Event、Organization 标记的正确属性名、在部署前验证 JSON-LD 代码片段以及跟进新的富媒体搜索结果类型。",
    ],
    slug: 'schema.org',
  ...
  // ══════════════════════════════════════════════════════
  {
    ...
    descriptionZh: [
      ...
      "Cursor 基于 VS Code 构建但增加了深度的 AI 集成，可与整个代码库对话、用上下文感知的提示生成功能、内联应用差异以及用\"Composer\"创建多文件更改。索引项目以提供贴合代码库的建议。",
      "不满足于简单自动补全的开发者的 AI 驱动编辑器常用。可用于更快上手不熟悉的代码库、通过描述期望行为实现功能、在 AI 引导下重构遗留代码以及提交前审查变更。",
    ],
    slug: 'cursor.com',
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for developers prototyping ideas, founders building MVPs, and designers creating interactive mockups. Use it to validate app concepts without writing boilerplate, generate quick internal tools, and create functional demos for stakeholder feedback.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for students learning to code, educators teaching programming, and developers who need a quick, disposable environment for experiments. Use it to prototype APIs, collaborate on coding interviews, run Python notebooks, and deploy simple web apps instantly.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    descriptionZh: [
      "成熟的 AI 文字转语音与声音克隆平台，生成逼真的旁白、配音和自定义语音，支持 29+ 语言，语音自然度行业领先。",
  ...
  },
  {
    ...
    titleZh: 'Gemini',
    description: [
      "Google's multimodal AI model with integrations across Google Workspace, Gmail, and Search, and support for text, images, code, and audio.",
      "Gemini works across Google Workspace, Gmail, Docs, Sheets, and Drive. It can reason over text, images, code, audio, and video, with long-context support for codebases and transcripts.",
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for founders validating startup ideas, designers creating functional prototypes without engineering support, and developers accelerating the early stages of new projects. The GitHub sync enables a smooth transition from AI generation to manual development.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    description: [
      "Stripe charges per-transaction and percentage-based fees, and the calculator shows how those charges affect net revenue.",
      "The calculator supports multiple regions with region-specific fee structures, including US, EU, UK, Australia, and more. Enter the transaction amount, select your region and account type, and instantly see the Stripe fee, net payout, and effective fee percentage.",
      "A practical tool for SaaS founders, ecommerce operators, and freelancers who process payments through Stripe. Use it to accurately forecast revenue after processing costs, set prices that account for payment fees, and compare Stripe costs across different regions.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "Amazon FBA 和 FBM 卖家从选品到启动到规模化各阶段都常用。可用于通过真实销售数据验证产品创意、追踪关键词排名、监控竞品库存以及在大规模备货前预估收入。",
    ],
  ...
  },
  {
    ...
    description: [
      ...
      "Useful for frontend learners bridging the gap between tutorials and professional work. Use it to practice translating designs to code, build a portfolio of polished projects, receive code reviews from other developers, and gain confidence with responsive layout, forms, and accessibility.",
    ],
    descriptionZh: [
  ...
  },
  {
    ...
    descriptionZh: [
      "程序员面试刷题平台，3000+ 算法题目，FAANG 等顶级科技公司求职者的数据结构和算法练习常用。",
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "每个 TypeScript 开发者的常用参考，从学习类型系统的初学者到查阅高级模式的资深工程师。可在编辑器中随时查阅以获得即时类型检查指导，也是 tsconfig 配置选项的权威来源。",
    ],
    slug: 'dev.to',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "构建数据驱动应用的前端开发者常用。可用于用最少代码获取、缓存和同步服务端状态、通过内置分页实现无限滚动以及以声明方式处理加载和错误状态。",
    ],
    slug: 'tanstack.com',
  ...
  },
  {
    ...
    descriptionZh: [
      "TypeScript 优先的 Schema 声明与验证库，一次定义即可同时获得静态类型推导和运行时校验，是 API 边界验证的常用方案。",
      "Zod 让你在一次声明中同时定义 TypeScript 类型和运行时校验 Schema，在 API 边界解析数据并自动从 Schema 推断静态类型。支持复杂校验、转换和错误消息，与 TypeScript 完全集成。",
      "处理外部数据的 TypeScript 项目常用，API 响应、表单输入、环境变量和文件解析。可用于在运行时验证 TypeScript 在编译时的保证，消除类型与现实之间的鸿沟。",
    ],
    slug: 'zod.dev',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "重视 Web 标准并想要精细控制请求/响应周期的开发者的强力框架。可用于构建快速、有弹性的 Web 应用，较高的 SEO、自动错误恢复的乐观 UI 以及在无 JavaScript 时仍可工作的基于表单的工作流。",
    ],
    slug: 'remix.run',
  ...
  },
  {
    ...
    descriptionZh: [
      ...
      "构建大规模、可维护后端服务的团队的 Node.js 框架常用。可用于创建结构良好的 REST 和 GraphQL API、通过内置传输层实现微服务架构以及在开发团队中强制执行一致的模式。",
    ],
    slug: 'nestjs.com',
  ...
  },
  {
    ...
    descriptionZh: [
      "响应式 UI 库，编译为直接 DOM 更新，React 式的开发体验加上无虚拟 DOM 的细粒度响应式，性能流畅。",
  ...
  },
  {
    ...
    titleZh: 'Payload CMS',
    description: [
      "Open-source headless CMS and application framework built on Next.js, code-first configuration, TypeScript-native, with a capable admin UI.",
      "A broad icon solution for projects that need more variety than smaller icon sets. Use it to build feature-rich dashboards, create icon-heavy admin panels, design mobile app navigation, and maintain visual consistency across a large product.",
